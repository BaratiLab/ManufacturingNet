...
class DNN(nn.Module):
    ...
    def _get_neuron_input(self):
        ...
        gate = 0
        while gate != 1:
            self.list_of_neurons = input('Please enter the number of neurons int input for each layer including input and output layers sequentially: \n (Example: 14(input layer), 128, 64, 32, 10(output layer): ')
            self.list_of_neurons = self.list_of_neurons.split(',')
            for i in range(len(self.list_of_neurons)):
                if self.list_of_neurons[i].isnumeric() and int(self.list_of_neurons[i]) >0 :            # checking numeric entries and correct values
                    gate = 1
                else:
                    gate = 0
                    break
    ...
    def _get_activation_input(self):
        ...
        while gate != 1:
            if self.default_gate == True:
                print('Default activation function ReLU selected for all the hidden layers')
                self.activations = ''
            ...
            if gate == 0:
                ...
                self.activation_list = [int(x) for x in self.activations if int(x)<7]                # checking the input for correct values
                self.activation_list = [int(x) for x in self.activation_list if int(x) >= 0]
                if len(self.activation_list) == (len(self.size_list)-2):                # checking the number of inputs
                    gate = 1
                else:
    ...
    def _get_batchnorm_input(self):
        ...
        while gate != 1:
            ...
            if gate == 0:
                ...
                self.batchnorm_list = [int(x) for x in self.batchnorms if int(x)<2]                 # checking the input for correct values
                self.batchnorm_list = [int(x) for x in self.batchnorm_list if int(x)>-1]
                if len(self.batchnorm_list) == (len(self.size_list)-2):                # checking the number of inputs
                    gate = 1
                else:
    ...
    def _get_dropout_input(self):
        ...
        while gate != 1:
            ...
            for i in range(len(self.dropout_values)):               # checking numeric entries
            ...
            if gate == 0:
                ...
                self.dropout_list = [float(x) for x in self.dropout_values if float(x) < 1]             # checking the input for correct values
                self.dropout_list = [float(x) for x in self.dropout_list if float(x) >= 0]
                if len(self.dropout_list) == (len(self.size_list)-2):               # checking the number of inputs
                    gate = 1
                else:
                    print('Please enter the dropout values for each hidden layers. Required entries: ', (len(self.size_list)-2), ' Entered valid entries: ', len(self.dropout_list))
                    print(' ')
                    gate = 0
        print(' ')
...
class DNNModel():
    ...
    def __init__(self,X,Y, shuffle = True):
        ...
        self.net = (DNN(self.default_gate)).double()             # building a network architecture
    ...
    def _get_valsize_input(self):
        ...
        while gate!= 1:
            if self.default_gate == True:
                ...
                self.valset_size = (input('Please enter the validation set size float input (size > 0 and size < 1) \n For default size, please directly press enter without any input: '))
