...
class BasicBlock(nn.Module):
    def __init__(self, in_channel,input_size,default =False):
        ...
        self.conv1=nn.Conv2d(in_channels=in_channel, out_channels=self.out_channel, kernel_size=self.kernel, stride=self.stride, padding=self.padding)
        self.input_size=conv2D_output_size(self.input_size,self.kernel,self.stride,self.padding)
        ...
        if default==False:
            ...
            if self.pooling_input:
                self.input_size=conv2D_output_size(self.input_size,(2,2),(2,2),(0,0))
    ...
    def get_dropout(self):
        ...
        gate = 0
        while gate != 1:
            self.dropout_value = input('Please enter a dropout value between 0 and 1 for this convolution. \n For default option of no dropout, please directly press enter without any input: ')
    ...
    def get_channel_input(self):
        ...
        gate=0
        while gate!=1:
            channel_input=(input('Please enter a value for number of out channels: '))
    ...
    def get_kernel_size(self):
        ...
        while gate!=1:
            self.kernel=[]
            kernel_input=(input('Please enter kernel size input (heigth,width)\nFor example 3,3\nFor default size, please directly press enter without any input: '))
            if kernel_input == '':
                print('Default Value selected')
                kernel_input = '3,3'
            ...
            if len(kernel_input)==2:
                for i in range(len(kernel_input)):
                    if kernel_input[i].isnumeric() and (1<= int(kernel_input[i]) <= 20): 
    ...
    def get_stride(self):
        ...
        while gate!=1:
            self.stride=[]
            stride_input = (input('Please enter stride input (heigth,width)\nFor example 1,1\nFor default size, please directly press enter without any input: '))
            if stride_input == '':
                print('Default Value selected')
                stride_input = '1,1'
            ...
            if len(stride_input)==2:
                for i in range(len(stride_input)):
                    if stride_input[i].isnumeric() and (1<= int(stride_input[i]) <= 20): 
    ...
    def get_padding(self):
        ...
        while gate!=1:
            self.padding=[]
            padding_input = (input('Please enter a value for padding (heigth,width)\nFor example 1,1\nFor default size, please directly press enter without any input: '))
            if padding_input == '':
                print('Default Value selected')
                padding_input= '0,0'
            ...
            if len(padding_input)==2:
                for i in range(len(padding_input)):
                    if padding_input[i].isnumeric() and (0<= int(padding_input[i]) <= 20): 
...
class Network(nn.Module):
    def __init__(self,img_size,num_class):
        ...
        self.fc1 = nn.Linear(self.in_channel*self.img_size[0]*self.img_size[1], 1024)   # fully connected layer, output k classes
...
class CNN2DImageModel():
    ...
    def __init__(self, train_data_address, val_data_address, shuffle = True):
        ...
        self.net = (Network(self.img_size, self.num_classes)).double()            # building a network architecture
    ...
    def get_default_paramters(self):
        ...
        gate = 0
        while gate!= 1:
            self.default = input('Do you want default values for all the training parameters (y/n)? ')
    ...
    def get_num_classes(self):
        ...
        if train_num_folder != val_num_folder :
            print('Warning: Number of folders in the Validation set and Training set is not the same.')
    ...
    def main(self):
        ...
        image_transform = transforms.Compose([transforms.Grayscale(num_output_channels=self.img_size[-1]), transforms.Resize((self.img_size[:-1]), interpolation=2), transforms.ToTensor()])

        self.train_dataset = torchvision.datasets.ImageFolder(root=self.train_address, transform = image_transform )            # creating the training dataset

        self.val_dataset = torchvision.datasets.ImageFolder(root=self.val_address, transform = image_transform )             # creating the validation dataset

        self.train_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size = self.batchsize, shuffle = True)           # creating the training dataset dataloadet

        self.dev_loader = torch.utils.data.DataLoader(self.val_dataset, batch_size = self.batchsize)           # creating the validation dataset dataloader
