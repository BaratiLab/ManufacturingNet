...
class CNN2D(nn.Module):
      ...
      while gate!=1:
        if drop_out.replace('.','').isdigit():
          if (float(drop_out) >= 0 and float(drop_out)<1):
            ...
            print("Please enter the valid numeric values. The value should lie between 0 and 1 [0, 1) ")
            drop_out = (input(("Please input the dropout probability")))
            gate = 0
        else:
          drop_out = (input(("Please input the dropout probability: ")))
  ...
  def make_CNN(self,block,n_conv,pool_list,drop,batch_norm_list): # Makes the CNN with forward pass
    ...
    self.linear_size = self.channels[-1] * shape[-1][0].item() * shape[-1][1].item()
...
class CNN_LSTM(nn.Module):
    ...
    def forward(self, x):
      ...
      lstm_input = torch.zeros((b,time,self.lstm.input_size)).double().to(self.device)
      for i in range(time):
...
class CNNLSTMModel():
    ...
    def __init__(self,X,Y, shuffle = True):
        ...
        self.lstm_network = LSTM(self.cnn_network.linear_size, self.default_gate).double()

        self._set_device()              # setting the device to gpu or cpu

        self.net = CNN_LSTM(self.cnn_network, self.lstm_network, self.device)             # building a network architecture
